      ...
      for (const file of filteredFiles.slice(0, this.maxFilesPerReview)) {
        ...
        try {
          ...
          if (review && review.comments.length > 0) {
            // Add commit_id and position for each review comment
          for (const comment of review.comments) {
            comment.commit_id = prDetails.head.sha; // commit hash for the PR
          }
            reviews.push(review);
...
*Generated by PR Code Review Utility*`;
  ...
  async postReviews(owner, repo, prNumber, review) {
    const allComments = review.flatMap(review => review.comments);
